...
import { z } from "zod";
import type { Credential, IssueResponse } from "./types.js";
...
function toStableId(obj: Credential) {
  const json = JSON.stringify(obj);
  return crypto.createHash("sha256").update(json).digest("hex");
}

async function replicateToVerification(payload: { credentialId: string; issuedAt: string; workerId: string }) {
  try {
    if (!REPLICATE_URL) return;
    ...
    console.error("replication failed:", (e as Error).message);
  }
}
...
app.post("/issue", async (req, res) => {
  ...
  const credential: Credential = parsed.data;
  ...
  type ExistingRow = { credential_id: string; issued_at: string; worker_id: string } | undefined;
  const existing = getStmt.get(credentialId) as ExistingRow;
  ...
  if (existing) {
    ...
    const resp: IssueResponse = { status: "already_issued", credentialId };
  ...
  const insertStmt = db.prepare(
    "INSERT INTO issued_credentials (credential_id, payload_json, issued_at, worker_id) VALUES (?, ?, ?, ?)"
  );
  insertStmt.run(credentialId, JSON.stringify(credential), issuedAt, workerId);

  const resp: IssueResponse = { status: "issued", credentialId, issuedAt, workerId };
